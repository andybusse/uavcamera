...
boolean takeSnapshot() {
  ...
  sSerial.println("sending setpackagesize");
  ...
  sSerial.print("Number of packages: ");
  sSerial.println(bufferSize);
  
  byte dataIn[bufferSize][512]; // 512 is from the setpackagesize command
  
  for(long package = 0;package<bufferSize;package++){
    
    sSerial.print("Sending ACK for package ");
    sSerial.println(package);
    sendACK(0x00,0x00,byte(package),byte(package>>8));
    
    // receive package
    for(int dataPoint = 0; dataPoint<512;dataPoint++){
      while (Serial.available() <= 0) {} // wait for data to be available n.b. will wait forever...
      dataIn[package][dataPoint] = Serial.read();
    }
    sSerial.print("Package ");
    sSerial.print(package);
