...
//---Prototypes---
	...
	uint8_t* ImageToByte (uint16_t* jpglengthOut, char *jpglocation);
...
//----MAIN FUNCTION----
		...
		int main(void)
		{
			...
			uint16_t jpegLength;
			uint8_t* jpegData = ImageToByte(&jpegLength, "H:\\eclipse\\workspace\\jpegCheaderExtractor_nonavr\\Debug\\nyanNyan.JPG");
			for(uint16_t i = 0; i < jpegLength; i++) {
				uint8_t glaah = jpegData[i];
			}
			//commandCheck(jpegData);
			return 0;
		}
...
//---ImageToByte function---
		...
		bool ImageToByte (uint16_t* jpglengthOut, char *jpglocation)
		{
			// Initialization
