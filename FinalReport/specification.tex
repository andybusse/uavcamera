...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Interrupt} the download of an incomplete image and allow the user to save the incomplete image. This was given a \textbf{low priority} as it does not affect basic functionality, but could potentially save the user some time.
		\item Select the \textbf{resolution settings} of the image. This was given a \textbf{low priority} as it does not affect basic functionality but would be a nice feature.
		...
		\item Allow the user to command the image capture to \textbf{trigger periodically} over a \textbf{userspecified time interval} will be added if time permits. This was given a \textbf{low priority} for the same reasons as the previous point.
	\end{itemize}
