...
//---Structures---
	...
	struct huffmanTable // HT structure
	{
		...
		HuffmanBitStringTable bitStringTable;
		HuffmanDecoderState decoderState;
...
//---Variables and structures by Header initialized in---
	...
	FixedLengthBitQueue decoderBitQueue;
...
//----MAIN FUNCTION----
	...
	FILE *jpegFile; // input stream
		...
		int main(void)
		{
			decoderBitQueue = fixedLengthBitQueue_new(15 + 8, decoderBitQueueArray);
			//Initialize input stream
			jpegFile = fopen("huff_simple0.jpg", "rb");
			if (jpegFile == NULL)
			{
			...
			// -- now attempt to decode some of the jpeg
		...
		uint8_t JPEGMethod(uint32_t JPEGCommand)
		{
			...
			switch(JPEGCommand)
			{
				...
				*/
				...
				{
					HTablePtr cHTables[(chrosize + 2) * 2];
					for (int i = 0; i < (chrosize + 2); i++) {
						uint8_t dcht = ImageDataScan(scanComponents, chrosize, i) / 16; // Obtain last 4 bits of the byte
						uint8_t acht = ImageDataScan(scanComponents, chrosize, i) % 16; // Obtain first 4 bits of the byte


						HTablePtr currHtPtr = startPtr;
						while(currHtPtr == NULL) {
							if(currHtPtr->HTnumber == dcht && currHtPtr->acdc == 0) {
								cHTables[i*2] = currHtPtr;
							} else if(currHtPtr->HTnumber == acht && currHtPtr->acdc == 1) {
								cHTables[(i*2) + 1] = currHtPtr;
							}
							currHtPtr = currHtPtr->nextPtr;
						}
					}
				}

				{ // begin scoping brackets
					HTable* currTable;
					currTable = startPtr;
					for(int i = 0; i < HTcount; i++) {
						currTable->bitStringTable = huffmanBitStringTable_build(currTable->symbolPtr, currTable->symbolNumber);
						for(int lengthLevel = 0; lengthLevel < 16; lengthLevel++) {

							for(int bitstringNum = 0; bitstringNum < currTable->symbolNumber[lengthLevel]; bitstringNum++) {
								printf("0x%X: ", currTable->symbolPtr[lengthLevel][bitstringNum]);
								if(lengthLevel < 8)
									printBitString(currTable->bitStringTable.bitStrings_0_7[lengthLevel][bitstringNum], lengthLevel);
								else
									printBitString(currTable->bitStringTable.bitStrings_7_15[lengthLevel - 8][bitstringNum], lengthLevel);
								printf("\n");
							}

						}
						currTable->decoderState = huffmanDecoder_new(&(currTable->bitStringTable), &decoderBitQueue);
						currTable = currTable->nextPtr;
					}
				} // end scoping brackets
								...
								if(newSPtr != NULL) // if space is available
								{
									...
									if ((newSPtr->acht != NULL) && (newSPtr->dcht != NULL) && (newSPtr->content != NULL))
									{
										...
										for (i = 0; i < chrosize + 2; i++)
										{
											...
											printf("0x%X acht: %d dcht: %d \n", currentByte, newSPtr->acht[i], newSPtr->dcht[i]);


											// using the huffman decoder, decode the image
